local fuelDecorRegistered = false

function EnsureFuelDecor()
	if fuelDecorRegistered then return end
	DecorRegister(Config.FuelDecor, 1)
	fuelDecorRegistered = true
end
	...
	QBCore = {
		Functions = {
			...
			GetPlayerData = function()
				if Config.FuelDebug then print("Fetching Player Data.") end
...
function PlayFuelSound(soundName, volume)
	if GetResourceState('interact-sound') == 'started' then
		TriggerServerEvent("InteractSound_SV:PlayOnSource", soundName, volume or 0.4)
	end
end
