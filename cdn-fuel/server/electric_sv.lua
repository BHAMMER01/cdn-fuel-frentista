...
RegisterNetEvent("cdn-fuel:server:electric:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	local src = source
	if not src then if Config.FuelDebug then print("SRC is nil!") end return end
	local player = QBCore.Functions.GetPlayer(src)
	if not player then if Config.FuelDebug then print("Player is nil!") end return end
