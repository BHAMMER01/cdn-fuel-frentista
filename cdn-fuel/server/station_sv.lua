if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    RegisterNetEvent('cdn-fuel:server:buyStation', function(location)
        ...
        if Config.FuelDebug then print(CitizenID) end
    ...
    RegisterNetEvent('cdn-fuel:station:server:fuelpickup:failed', function(location)
        ...
        if location then
            if FuelPickupSent[location] then
                ...
                if Config.FuelDebug then print("User encountered an error with fuel pickup, so we are updating the fuel level anyways, and cancelling the pickup. SQL Execute Update: fuel_station level to: "..FuelPickupSent[location].refuelAmount.. " | Source: "..src.." | Citizen Id: "..cid..".") end
    ...
    if Config.Core ~= "ESX" then
        ...
        lib.callback.register('cdn-fuel:server:fetchinfo', function(source, location)
            ...
            if result[1] then
                if Config.FuelDebug then print(json.encode(result)) end
                if Config.FuelDebug then print("returning:" ..json.encode(result)) end
                return result
            else
                return false
            end
        ...
        if Config.FuelDebug then print("Registered Callback: ".."cdn-fuel:server:fetchinfo") end
    end
